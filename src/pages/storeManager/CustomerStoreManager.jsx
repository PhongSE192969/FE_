...
const CustomerStoreManager = () => {
  ...
  const getTierBadge = (tier) => {
    ...
    const currentStyle = styles[normalizedTier] || styles.BRONZE;

    return (
      <span
        className={`text-[10px] font-black px-2 py-0.5 rounded-md border ${currentStyle}`}
      >
        {normalizedTier}
      </span>
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Quản lý hồ sơ và lịch sử mua hàng của khách hàng
          </p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        >
          {customers.map((customer, index) => {
            ...
            const username = customer.username || "unknown";
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      <span className="text-sm font-bold text-indigo-600">
                        {customer.currentPoint ?? 0} pts
                      </span>
                      <span className="text-[10px] text-gray-400">
                        Tổng: {customer.totalPoint ?? 0}
                      </span>
                    </div>
                  </td>
                )}
...
export default CustomerStoreManager;