...
import { ConfirmModal, CustomerDetailModal } from "@/components/modal";
import {
  Eye,
  MoreVertical,
  Activity,
...
const CustomersManagement = () => {
  ...
  const tableColumns = useMemo(
    () => [
      ...
      { id: "contact", label: "Liên hệ", sortable: false },
      { id: "loyalty", label: "Hạng & Điểm", sortable: false },
      {
        id: "status",
        label: "Trạng thái",
        sortable: true,
  ...
  const executeDeleteUser = async () => {
    ...
    try {
      const customerId =
        deleteConfirmUser.customerFranchiseId || deleteConfirmUser.id;

      await customerService.deleteCustomer(customerId);

      toast.success("Đã vô hiệu hóa khách hàng thành công");
  ...
  const handleUpdateStatus = async (customer, newStatus) => {
    try {
      const customerId = customer.customerFranchiseId || customer.id;
  ...
  return (
    <div className="space-y-6 pb-10 p-6 bg-gray-50 min-h-screen font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-900">
            Khách Hàng & Thành Viên (Toàn Hệ Thống)
          </h2>
          <p className="text-gray-500 text-sm mt-1">
            Quản lý hồ sơ, trạng thái và hạng thẻ của toàn bộ khách hàng
          </p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        >
          {customers.map((customer, index) => {
            ...
            const menuKey = customer.customerFranchiseId || customer.id;
            const fullName = customer.fullName || "Guest User";
            const avatarUrl = customer.avatarUrl;
            const status = customer.userStatus || customer.status || "ACTIVE";
              ...
              >
                {visibleColumns.includes("index") && (
                  <td className="px-6 py-5 text-sm font-bold text-gray-400">
                    {(pagination.page - 1) * size + index + 1}
                  </td>
                )}
                  ...
                  <td className="px-6 py-5 text-right relative">
                    ...
                    >
                      <button
                        ...
                        className="p-2 text-gray-400 hover:text-[#d9a13b] hover:bg-white rounded-lg transition-all"
                        title="Xem chi tiết"
                      >
                        <Eye size={16} />
                      </button>
                      ...
                      <button
                        onClick={() =>
                          setOpenActionMenuId(
                            openActionMenuId === menuKey ? null : menuKey
                          )
                        }
                      ...
                      {openActionMenuId === menuKey && (
                        <div className="absolute right-10 top-10 w-40 bg-white border border-gray-100 shadow-xl rounded-xl py-2 z-[50] flex flex-col text-sm">
                          {status === "ACTIVE" ? (
                            <button
                              onClick={() =>
                                handleUpdateStatus(customer, "INACTIVE")
                              }
                              className="w-full text-left px-4 py-2 hover:bg-orange-50 text-orange-600 font-bold"
                            >
                              Set Inactive
                            </button>
                          ) : (
                            <button
                              ...
                              className="w-full text-left px-4 py-2 hover:bg-green-50 text-green-600 font-bold"
                            >
                              Set Active
                            </button>
                          )}
      ...
      <ConfirmModal
        ...
        message={`Bạn có chắc chắn muốn vô hiệu hóa khách hàng ${
          deleteConfirmUser?.fullName || "này"
        }?`}
        confirmText="Xóa"
        type="danger"
      />
